var menu = {
	...
	render: function() {
		var d = $('<div></div>');
		if (this.arch.folders && this.arch.folders.length) {
			$.each(this.arch.folders, function(i, val) {
				d.append(menu__li(val));
			});
		}
		if (this.arch.files && this.arch.files.length) {
			$.each(this.arch.files, function(i, val) {
				d.append(menu__li(val));
			});
		}
		this.$menu.append(d.html());

		function menu__li(param) {
			var focus_class = ($('#menu .menu__link.menu__focused').data('id') === param.id) ? ' menu__focused' : '';
			var li = $('<li class="menu__ menu__' + param.type + '"></li>');
			var link = $('<a class="menu__link menu__link_' + param.type + focus_class + '" data-id=' + param.id + ' href="#">' + param.name + '</a>');
			link.prepend(menu.svg_icon(param.type));
			li.append(link);
			// console.log(123, this);
			var ul = $('<ul class="menu_sub"></ul>');
			if (param.folders && param.folders.length) {
				$.each(param.folders, function(i, val) {
					ul.append(menu__li(val));
				});
			}
			if (param.files && param.files.length) {
				$.each(param.files, function(i, val) {
					ul.append(menu__li(val));
				});
			}
			if (!ul.html()) {
